...
public class RLContext : DbContext
{
    ...
    public DbSet<PlanProcedureUser> PlanProceduresUsers { get; set; }
    ...
    protected override async void OnModelCreating(ModelBuilder builder)
    {
        ...
        {
            typeBuilder.HasKey(pu => new { pu.PlanId, pu.ProcedureId, pu.UserId });
            typeBuilder.HasOne(pu => pu.Plan).WithMany(p => p.PlanProceduresUsers);
            typeBuilder.HasOne(pu => pu.Procedure).WithMany(p => p.PlanProcedureUsers);
            typeBuilder.HasOne(pu => pu.User).WithMany();
        });
